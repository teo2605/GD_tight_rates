classdef pep < handle
    ...
    methods
        ...
        function obj=TraceHeuristic(obj,active)
            assert(active==0 || active==1 || active == 2,'Trace heuristic option should be 0/1/2');
...
%                     names{end+1} = sprintf('Perf%d',i);
                    names{end+1} = sprintf('Perf%d',i-1); % to start from 0
                end
...
%                 S_
            ...
            if obj.pesto_opt_traceheuristic % to obtain "low-dimensional" wc functions
                if obj.list_size_perf > 0                    
                    ...
                    tol_dimension_reduction = 1e-8;
                    cons = cons + (obj_func >= out.WCperformance - tol_dimension_reduction);
                    names = [names 'Fix'];                             
                    if obj.pesto_opt_traceheuristic == 1 %% trace
                        optimize(cons,trace(G),solver_opt);
                        [nb_eigenvalues, eig_threshold, corrected_G_value] = obj.get_nb_eigenvalues_and_corrected_matrix(double(G));
                    else %% logdet                                                                
                        %%                        
                        GG = double(G); 
                        [~, ~, corrected_G_value] = obj.get_nb_eigenvalues_and_corrected_matrix(GG);                        
                        %%
                        niter = 40; eig_regularization = 1e-7;
                        for i = 1 : niter
                            W_inv = corrected_G_value + eig_regularization * eye(dim1);                            
                            optimize(cons,trace(W_inv \ G),solver_opt);
                            GG = double(G);
                            [nb_eigenvalues, eig_threshold, corrected_G_value] = obj.get_nb_eigenvalues_and_corrected_matrix(GG);
                            if mod(i,10) == 0
                                fprintf("iteration %d, eig_regularization %.2e: rank = %d \n\n", ...
                                        i, eig_regularization, nb_eigenvalues);
                            end
                            % Print the estimated dimension after i dimension reduction steps                                                        
                        end
                    end                    
                    fprintf("Final: rank = %d \n\n", rank(double(corrected_G_value)) );
                    verbose_pet = true;
                    if verbose_pet
                       fprintf(' PESTO: Solver output: %7.5e, solution status: %s\n',out.WCperformance,out.solverDetails.info);
                       fprintf(' PESTO Postprocessing: %7.5e eigenvalue(s) > %7.5e after %d dimension reduction step(s) \n',...
                            nb_eigenvalues, eig_threshold, i);
                    end
                    out.G_low = corrected_G_value;
                    out.TraceG = trace(corrected_G_value);
                    out.Gram = corrected_G_value;
                else
            ...
            if obj.list_size_perf > 0 || obj.pesto_opt_traceheuristic > 0
                ...
                % Decomposition
                if obj.pesto_opt_traceheuristic
                    G_ = out.G_low;
                else
                    G_ = double(G);
                end
                G_ = (G_ + G_') / 2;
%                 [V,D]=eig(double(G));%
                [V,D]=eig(G_);% [V,D] = eig(G_(2:end,2:end));
        ...
        function [nb_eigenvalues, eig_threshold, corrected_S] = get_nb_eigenvalues_and_corrected_matrix(obj, M)
        %%%%
        ...
        eig_threshold = max( max(eig_val)/1e3, 2 * max(-eig_val) );

        % Correct eig_val accordingly.
        non_zero_eig_vals = eig_val >= eig_threshold;
        nb_eigenvalues = sum(non_zero_eig_vals);
        corrected_eig_val = non_zero_eig_vals .* eig_val;

        % Recompute M (or S) accordingly.
        corrected_S = eig_vec * diag(corrected_eig_val) * eig_vec';
        corrected_S = (corrected_S + corrected_S') / 2;

        % Get the highest eigenvalue that have been set to 0.
        eig_threshold = 0;
        if ~isempty ( max(eig_val(non_zero_eig_vals == 0)) )
            eig_threshold = max( eig_val(non_zero_eig_vals == 0) );
        end
        end
    end
    methods (Static)
